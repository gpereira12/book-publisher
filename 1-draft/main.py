...
1-draft/main.py
-----------------
Motor do Draft (Projeto 1 — Escrita): Gerador de manuscritos estruturados em Markdown + YAML.
Implementa:
...
def generate_cinematic_image_prompts(config: Dict[str, Any], output_dir: Path) -> Path:
    """[Isolado por Projeto] Gera a biblioteca de prompts cinematográficos para o Cover (Projeto 4)."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Draft (Projeto 1): Escrita - Motor de Geração Literária")
    parser.add_argument("--config", default="1-draft/book_config.sample.yaml", help="Caminho do YAML de configuração")
    parser.add_argument("--output", help="Caminho de saída para o arquivo .md gerado")
    args = parser.parse_args()

    config_path = Path(args.config).resolve()
    print(f"✍️ [Draft] Carregando configuração: {config_path}")
    ...
    # 3. Gerar Prompts Cinematográficos para o Cover (Projeto 4)
