...
2-revisao/main.py
-----------------
Motor do Projeto 2 (Revisão): Auditador e Lapidador Editorial Profissional.
Aplica as 4 Camadas de Revisão + Folha de Estilo + Verba Dicendi + Auto-Disparo do Projeto 3.
"""
...
def generate_review_report(
    ...
    lines.extend([
        ...
        "O manuscrito passou por todas as Camadas de Revisão e está **APROVADO** para o Projeto 3 (Diagramação)."
    ])

    return "\n".join(lines)


def trigger_project_3_diagramming(book_id: str) -> None:
    """[Sugestão 3] Dispara a execução automática do Projeto 3 (Diagramação)."""
    print(f"\n🚀 [--auto-approve] Disparando Projeto 3 (Diagramação) para '{book_id}'...")
    ...
    try:
        ...
        print("✨ [Projeto 3] Diagramação executada automaticamente com sucesso!")
    except subprocess.CalledProcessError as err:
        print(f"❌ [Erro] Falha ao executar o Projeto 3 automaticamente: {err}", file=sys.stderr)


def main() -> None:
    parser = argparse.ArgumentParser(description="Projeto 2: Revisão Editorial")
    parser.add_argument("--book-dir", required=True, help="Nome da pasta do livro em inputs/")
    parser.add_argument("--auto-approve", action="store_true", help="Dispara o Projeto 3 (Diagramação) automaticamente após revisão")
    ...
    print(f"🔍 [Projeto 2] Auditando manuscrito: {md_file}...")
    ...
    print(f"✨ [Projeto 2] Manuscrito lapidado salvo em: {revisado_file}")
    ...
    print(f"📊 [Projeto 2] Relatório de revisão gerado em: {report_file}")

    # 7. Disparo Automático do Projeto 3 se ativado (Sugestão 3)
    if args.auto_approve:
        trigger_project_3_diagramming(args.book_dir)
